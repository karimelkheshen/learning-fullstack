...
attendee_bp = Blueprint("attendees", __name__, url_prefix="/attendees")
...
def add_attendee_route():
    ...
    try:
        ...
        return (
            jsonify({"status": "validation_error", "data": {"details": e.errors()}}),
            400,
        )
...
def get_all_attendees_route():
    ...
    return jsonify({"status": "ok", "data": {"attendees": attendees}}), 200


@attendee_bp.get("/<string:attendee_id>")
def get_attendee_by_id_route(attendee_id: str):
    ...
    return jsonify({"status": "ok", "data": {"attendee": found_attendee}}), 200


@attendee_bp.delete("/<string:attendee_id>")
